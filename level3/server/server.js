...
exports = {
  ...
  onTicketCreateHandler: function (args) {
    $request
      .post(`https://api.github.com/repos/${args.iparams.github_repo}/issues`, {
        headers: {
      ...
      .then(
        (data) => {
          console.info(
            "Successfully created the GitHub issue for the Freshdesk ticket"
          );
          saveMapping({
            ticketID: args.data.ticket.id,
            issueID: data.response.id,
            issueNumber: data.response.number,
          }).then(
            function () {
        ...
        (error) => {
          console.error(
            "Error: Failed to create the GitHub issue for the Freshdesk ticket"
          );
          console.error(error);
        }
      );
